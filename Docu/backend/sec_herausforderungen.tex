...
\textbf{Lösung:} Einführung einer \texttt{BackgroundJobQueue}. Der Upload-Endpoint nimmt die Datei entgegen, speichert sie in MinIO und reiht nur eine \textquotedbl Job-ID\textquotedbl in die Queue ein, bevor er sofort \texttt{202 Accepted} zurückgibt. Ein Hintergrunddienst ("Worker") arbeitet diese Queue asynchron ab.
...
\textbf{Lösung:} Implementierung des \texttt{TokenService} mit \textit{Tiktoken}. Bevor ein Prompt generiert wird, werden die Tokens der abgerufenen Chunks gezählt. Ist das Limit erreicht, werden die am wenigsten relevanten Chunks verworfen (\textquotedbl Smart Truncation\textquotedbl).
