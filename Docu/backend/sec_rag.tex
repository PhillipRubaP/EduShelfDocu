...
Das Herzst\"uck von EduShelf ist die RAG-Pipeline \cite{lewisRAG2020}: Anstatt das LLM allein auf sein allgemeines Wissen zu st\"utzen, wird jede Benutzeranfrage zun\"achst dazu genutzt, relevante Textabschnitte aus den hochgeladenen Dokumenten abzurufen. Diese Abschnitte werden dann als \textit{Kontext} in den Prompt eingef\"ugt, bevor das LLM zur Antwort aufgefordert wird. Dies reduziert Halluzinationen und erm\"oglicht präzise sowie dokumentspezifische Antworten.
...
\begin{figure}[H]
    ...
    \caption{Architekturdiagramm des RAG-Systems in EduShelf (Quelle: \cite{promptingguideRAG})}
...
\begin{itemize}
    \item \texttt{Type}: Klassifiziert die Anfrage als \texttt{\dq{}question\dq{}}, \texttt{\dq{}summarize\dq{}}, \texttt{\dq{}flashcards\dq{}} oder \texttt{\dq{}quiz\dq{}}.
...
\begin{itemize}
    ...
    \item Den \textbf{bisherigen Konversationsverlauf} der aktuellen Chat-Sitzung (f\"ur kontextsensitive Folgefragen).
...
\subsection{Zusammenfassung: Retrieval-Augmented Generation \cite{lewisRAG2020}}
