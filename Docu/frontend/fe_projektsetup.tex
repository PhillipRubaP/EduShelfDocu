...
Das Projekt wurde mit dem Befehl \texttt{npm create vite@latest} initialisiert~\cite{ViteDocs}. Die Konfiguration befindet sich in der Datei \texttt{vite.config.js}.
...
Ein kritischer Aspekt dieser Konfiguration ist der \textbf{Proxy}: Um während der Entwicklung Cross-Origin Resource Sharing (CORS)~\cite{MDNCORS} Probleme zu vermeiden, leitet der Vite-Entwicklungsserver alle Anfragen, die mit \texttt{/api} beginnen, an das Backend (Port 5015) weiter. Die Option \texttt{changeOrigin: true} manipuliert dabei den Host-Header der Anfrage so, als käme sie direkt vom Backend-Server selbst.
