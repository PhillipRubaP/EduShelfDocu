...
\begin{lstlisting}[language=JavaScript, caption={Datenstruktur eines Quiz}]
...
Beim Ändern eines Antworttextes darf der State nicht direkt angepasst werden. Stattdessen muss eine Kopie der betroffenen Ebenen erstellt werden -- das Prinzip der Immutability~\cite{ReactImmutability} in React:
...
Beim Speichern (\texttt{handleSubmit}) wird unterschieden, ob ein neues Quiz erstellt (\texttt{POST}) oder ein bestehendes bearbeitet (\texttt{PUT}) wird. Nach erfolgreicher API-Antwort ruft das Modal den übergebenen Callback \texttt{onQuizSaved()} auf, damit die Parentkomponente ihre Liste ohne erneuten Fetch aktualisiert.
